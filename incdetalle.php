...
<div style="clear:both; overflow:hidden;">
	    <?php
		  ...
		  while($info = $aclass->respuesta()){
			 echo '<div class="left" '.((($i%4)==0)?'style="margin-right:0;"':'').'><div class="celdadet" '.((($i%4)==0)?'style="padding-right:0;"':'').'><a class="segura" id="img" href="'.CodificaUrlImg($contenedor.$info['cimggaleria']).'"><img class="segura" src="'.CodificaUrlImg($contenedor.str_replace('fotos','thumbs',$info['cimggaleria'])).'"  width="154" height="116"/></div></div>';
			 $i++;
		  }
		?>
</div>
